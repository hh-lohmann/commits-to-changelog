...
suite('Feature: Write newest changelog entries also to README.md',()=>{
  suite('Setting linesToReadme: Type',()=>{
    ...
    test( 'linesToReadme must not be a float number',()=> {
      ...
      _lib.cd(tstdir);
      _lib.gitMockCommit();
      writeFileSync('package.json','{"commits-to-changelog":{"commitMoveTag":false,"linesToReadme":0.1,"requireTag":false}}');
      assert(
    ...
    test( 'linesToReadme must not be a negative integer',()=> {
      ...
      _lib.cd(tstdir);
      _lib.gitMockCommit();
      writeFileSync('package.json','{"commits-to-changelog":{"commitMoveTag":false,"linesToReadme":-1,"requireTag":false}}');
      assert(
        _lib.commonSpawnErrContains('must be a positive integer',_lib.runPkgCli()),
        'see '+tstdir
      );
    ...
    test( 'linesToReadme must not be a string containing no integer',()=> {
      ...
      _lib.cd(tstdir);
      _lib.gitMockCommit();
      writeFileSync('package.json','{"commits-to-changelog":{"commitMoveTag":false,"linesToReadme":"pure string","requireTag":false}}');
      assert(
        _lib.commonSpawnErrContains('must be a positive integer',_lib.runPkgCli()),
        'see '+tstdir
      );
  ...
  test( 'If no tag is given: Use date',()=> {
    ...
    writeFileSync('package.json','{"commits-to-changelog":{"commitMoveTag":false,"linesToReadme":5,"requireTag":false}}');
    _lib.runPkgCli();
